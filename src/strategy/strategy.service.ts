...
import { DataService, MappedDerivative, MappedEquity } from "src/data/data.service";
...
import { OrderManagarService } from "src/order-manager/order-manager.service";
import { OMOrder } from "src/order-manager/types";
...
export class StrategyService {
    ...
    constructor(
        ...
        private readonly orderManagerService: OrderManagarService,
    ) { }
    private readonly eventEmitter: EventEmitter2,

    async initialize() {
        this.portfolioService.getHoldings().forEach((({ token }) => this.tokens.add(token)))
    ...
    getStrategyTag(tradingsymbol: EquityTradingsymbol) {
    ...
    shouldMakeDecision({ price, token }: Tick): boolean {
        ...
        const position = positions.filter((position) => position.name === equity.tradingsymbol)[0]

        // redundent check
        if(positions.length > 1) {
            console.log(`fatal error: cannot make a decision as there are multiple positions for the underlying: ${equity.tradingsymbol}`)
            return false
        }

        // LTP > CALL STRIKE + CALL PREMIUM
        if( price > DataService.parseDerivativeTradingSymbol(position.tradingsymbol).strike +  position.averagePrice) {
            return true
        }
    ...
    private tooCloseToExpiry() {
        return this.dataService.hasNDaysToExpiry(this.executionContext.availableOTMCallOption.tradingsymbol, 3)
    }
    ...
    private getTarget(equityTradingsymbol: EquityTradingsymbol) {
        ...
        const holdings = this.portfolioService.getHoldingsForEquity(equityTradingsymbol)
    ...
    private updateStrategyContext({ token, price }: Tick) {
        ...
        this.executionContext = {
            ...
            currentMonth: month,
            equityInfo, 
            existingCallOptions: this.portfolioService.getCallPositionsForEquityAndMonth(equityInfo.tradingsymbol, month),
            availableOTMCallOption: this.dataService.getAvailableOTMCallOptionFor(equityInfo.tradingsymbol, DataService.getToday().month, price),
    ...
    private isTriggerHit() {
        ...
        const { strike } = DataService.parseDerivativeTradingSymbol(this.executionContext.existingCallOptions[0].tradingsymbol)
    ...
    private resync(holdings, positions, balance) {
        ...
        const ltpRecord = (await this.apiService.getDerivativeLtp(this.executionContext.availableOTMCallOption.tradingsymbol))[this.executionContext.equityToken]

        this.updateStrategyContext({token: ltpRecord.instrument_token, price: ltpRecord.last_price})
    }

    private waitForOrderManagerUnlock(): Promise<void> {
        return new Promise((resolve) => {
            this.eventEmitter.once('order-manager.unlocked', resolve);
        });
    }

    private async sellNewOTMCall() {
        const ltpRecord = await this.apiService.getDerivativeLtp(this.executionContext.availableOTMCallOption.tradingsymbol)
        const newOrder: OMOrder = {
            tradingsymbol: this.executionContext.availableOTMCallOption.tradingsymbol,
            token: this.executionContext.availableOTMCallOption.token,
            price: ltpRecord[this.executionContext.availableOTMCallOption.tradingsymbol].last_price,
            quantity: 0,
            buyOrSell: "SELL",
            tag: this.executionContext.strategyTag
        }
        this.executionContext.orders.push(newOrder)
        this.orderManagerService.execute(this.executionContext.orders)

        await this.waitForOrderManagerUnlock()
    }
